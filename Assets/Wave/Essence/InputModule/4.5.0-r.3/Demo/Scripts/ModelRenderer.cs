...
namespace Wave.Essence.InputModule.Demo
{
	...
	public class ModelRenderer : MonoBehaviour
	{
		...
		public HandManager.TrackerType Tracker = HandManager.TrackerType.Electronic;
		...
		void Awake()
		{
			InitializeModel();
			/// Engine++
			result = new JointResult();
			result.rotation = Quaternion.identity;
			result.points = new Vector3[21];
			for (int i = 0; i < 21; i++)
				result.points[i] = Vector3.zero;
			result.confidence = 0;
			/// Engine --
		}

		/// Engine++
		void UpdateResults(bool isLeft)
		{
			if (HandManager.Instance == null || !HandManager.Instance.IsHandPoseValid(Tracker, isLeft))
				return;

			HandManager.Instance.GetJointRotation(Tracker, HandManager.HandJoint.Wrist, ref result.rotation, isLeft);
			HandManager.Instance.GetJointPosition(Tracker, HandManager.HandJoint.Wrist, ref result.points[0], isLeft);

			HandManager.Instance.GetJointPosition(Tracker, HandManager.HandJoint.Thumb_Joint0, ref result.points[1], isLeft);
			HandManager.Instance.GetJointPosition(Tracker, HandManager.HandJoint.Thumb_Joint1, ref result.points[2], isLeft);
			HandManager.Instance.GetJointPosition(Tracker, HandManager.HandJoint.Thumb_Joint2, ref result.points[3], isLeft);
			HandManager.Instance.GetJointPosition(Tracker, HandManager.HandJoint.Thumb_Tip, ref result.points[4], isLeft);

			HandManager.Instance.GetJointPosition(Tracker, HandManager.HandJoint.Index_Joint1, ref result.points[5], isLeft);
			HandManager.Instance.GetJointPosition(Tracker, HandManager.HandJoint.Index_Joint2, ref result.points[6], isLeft);
			HandManager.Instance.GetJointPosition(Tracker, HandManager.HandJoint.Index_Joint3, ref result.points[7], isLeft);
			HandManager.Instance.GetJointPosition(Tracker, HandManager.HandJoint.Index_Tip, ref result.points[8], isLeft);

			HandManager.Instance.GetJointPosition(Tracker, HandManager.HandJoint.Middle_Joint1, ref result.points[9], isLeft);
			HandManager.Instance.GetJointPosition(Tracker, HandManager.HandJoint.Middle_Joint2, ref result.points[10], isLeft);
			HandManager.Instance.GetJointPosition(Tracker, HandManager.HandJoint.Middle_Joint3, ref result.points[11], isLeft);
			HandManager.Instance.GetJointPosition(Tracker, HandManager.HandJoint.Middle_Tip, ref result.points[12], isLeft);

			HandManager.Instance.GetJointPosition(Tracker, HandManager.HandJoint.Ring_Joint1, ref result.points[13], isLeft);
			HandManager.Instance.GetJointPosition(Tracker, HandManager.HandJoint.Ring_Joint2, ref result.points[14], isLeft);
			HandManager.Instance.GetJointPosition(Tracker, HandManager.HandJoint.Ring_Joint3, ref result.points[15], isLeft);
			HandManager.Instance.GetJointPosition(Tracker, HandManager.HandJoint.Ring_Tip, ref result.points[16], isLeft);

			HandManager.Instance.GetJointPosition(Tracker, HandManager.HandJoint.Pinky_Joint1, ref result.points[17], isLeft);
			HandManager.Instance.GetJointPosition(Tracker, HandManager.HandJoint.Pinky_Joint2, ref result.points[18], isLeft);
			HandManager.Instance.GetJointPosition(Tracker, HandManager.HandJoint.Pinky_Joint3, ref result.points[19], isLeft);
			HandManager.Instance.GetJointPosition(Tracker, HandManager.HandJoint.Pinky_Tip, ref result.points[20], isLeft);

			result.confidence = HandManager.Instance.GetHandConfidence(Tracker, IsLeft);
		}
		/// Engine --

		void Update()
		{
			UpdateResults(IsLeft);
